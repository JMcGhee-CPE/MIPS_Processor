...
def compare(student_file_path, mars_file_path, max_mismatches=2, outfunc=print, help=True):
    ...
    while cur_mismatches < max_mismatches:
        ...
        #Student and mars wrote to either register file or memory:
            ...
            if student_reg_write == mars_reg_write:
                if mars_reg_write:
                    if not (mars_secondline_search.group('val').upper() == student_secondline_search.group('val').upper() and mars_secondline_search.group('reg').upper() == student_secondline_search.group('reg').upper()):
                        cur_mismatches = cur_mismatches + 1
                        if cur_mismatches == 1:
                            outfunc('Oh no...\n')
                        outfunc(f'Cycle: {student_firstline_search.group("cycle")}')
                    ...
                    if not (mars_secondline_search.group('val').upper() == student_secondline_search.group('val').upper() and mars_secondline_search.group('addr').upper() == student_secondline_search.group('addr').upper()):
                        cur_mismatches = cur_mismatches + 1
                        if cur_mismatches == 1:
                            outfunc('Oh no...\n')
                        outfunc(f'Cycle: {student_firstline_search.group("cycle")}')
