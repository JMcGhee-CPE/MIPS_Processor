...
def check_vhdl_present():
    ...
    non_vhd = next((f for f in src_dir if f.endswith('.v')),None)
    if non_vhd:
    ...
    expected = {'tb.v','mem.v','MIPS_Processor.v'}

    # return True if at least 1 new .vhd file exists
    is_student_vhd = lambda f: f.endswith('.v') and f not in expected
